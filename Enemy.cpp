...
void Enemy::shoot(float)
{
    ...
    bullet->setPosition(Vec2(pos.x, pos.y - getContentSize().height * 0.2));
}
...
//    
    ...
    if (Config::sharedConfig()->getAudioState()) {
            SimpleAudioEngine::getInstance()->playEffect(s_explodeEffect);
    }
}
...
{
    SpriteFrameCache::getInstance()->addSpriteFramesWithFile(s_Enemy_plist, s_enemy);
}